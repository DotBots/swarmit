...
bool localization_get_position(position_2d_t *position) {
    if (LH2_CALIBRATION_IS_VALID) {
        ...
        if (_distance((position_2d_t *)&_localization_data.previous_position, (position_2d_t *)&_localization_data.position) > VALID_POSITION_DISTANCE_THRESHOLD_MM) {
            printf("Position jump too high from (%u,%u) to (%u,%u)\n",
        ...
        position->y = _localization_data.position.x;
